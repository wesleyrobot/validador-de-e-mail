            ...
            try:
                ...
                if status.get('status') == 'done':
                    ...
                    with col1:
                        ...
                        st.markdown("[⬇️ Baixar Excel](http://138.197.145.84/api/download/" + job_id + "?format=xlsx)")
                        st.markdown("[⬇️ Baixar CSV](http://138.197.145.84/api/download/" + job_id + "?format=csv)")
                    
                    with col2:
                        st.markdown("### 📧 Nome + E-mail")
                        st.markdown("[⬇️ Baixar Excel Filtrado](http://138.197.145.84/api/download/" + job_id + "/filtered?format=xlsx&valid_only=true)")
                        st.markdown("[⬇️ Baixar CSV Filtrado](http://138.197.145.84/api/download/" + job_id + "/filtered?format=csv&valid_only=true)")
